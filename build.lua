...
local function download_jdk(config)
   ...
   if XBUILD.exists(jdk_dir .. "/bin/java.exe") then
   ...
   if config.os_name == OS_NAME.WINDOWS then
      ...
      XBUILD.run("tar -xf jdk.zip -C dist\\tools\\jdk")

      -- Normalize folder name to /windows
      XBUILD.run("for /d %i in (dist\\tools\\jdk\\jdk-*) do move %i dist\\tools\\jdk\\windows")

   elseif config.os_name == OS_NAME.LINUX then
      ...
      XBUILD.run("mv dist/tools/jdk/jdk-* " .. jdk_dir)
...
local function build_app(config)
   ...
   "echo %CD% && " ..
   "echo %~dp0 && " ..
   "dir dist\\tools\\jdk\\windows\\bin\\java.exe"
)
      XBUILD.run(
         "set JAVA_HOME=%~dp0dist\\tools\\jdk\\windows && " ..
...
local function package(config)
   if config.os_name == OS_NAME.WINDOWS then
      XBUILD.run("mkdir dist\\app")
      XBUILD.run("mkdir dist\\lib")
      XBUILD.run("mkdir dist\\plugins")

      XBUILD.run("copy xide-app\\target\\xide-app-0.1.0.jar dist\\app\\xide.jar")

      XBUILD.run("copy xide-core\\target\\xide-core-0.1.0.jar dist\\lib\\")
      XBUILD.run("copy xide-ui\\target\\xide-ui-0.1.0.jar dist\\lib\\")
      ...
      XBUILD.run("copy xide-standard-plugins\\xide-folder-tree-plugin\\target\\xide-folder-tree-plugin-0.1.0.jar dist\\plugins\\")
      XBUILD.run("copy xide-standard-plugins\\xide-terminal-plugin\\target\\xide-terminal-plugin-0.1.0.jar dist\\plugins\\")
      XBUILD.run("copy xide-standard-plugins\\xide-settings-plugin\\target\\xide-settings-plugin-0.1.0.jar dist\\plugins\\")
      XBUILD.run("copy xide-standard-plugins\\xide-rust-file-plugin\\target\\xide-rust-file-plugin-0.1.0.jar dist\\plugins\\")
      XBUILD.run("copy xide-standard-plugins\\xide-logs-plugin\\target\\xide-logs-plugin-0.1.0.jar dist\\plugins\\")
